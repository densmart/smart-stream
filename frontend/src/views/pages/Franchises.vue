...
const breadcrumbs = ref([{ label: 'All Franchises', id: null }]);
...
const loadPlaylists = async () => {
    try {
        ...
        toast.add({
            severity: 'error',
            summary: 'Error',
            detail: error.response?.data?.error || 'Failed to load franchises',
            life: 3000
        });
    } finally {
        loading.value = false;
...
const searchPlaylists = async (event) => {
    try {
        ...
        const results = await playlistsApi.searchPlaylists(query || undefined, PlaylistType.FRANCHISE);
        filteredPlaylists.value = results;
    } catch (error) {
        toast.add({
            severity: 'error',
            summary: 'Error',
            detail: 'Failed to search franchises',
            life: 3000
        });
    ...
    try {
        ...
        if (isEditMode.value) {
            ...
            toast.add({
                severity: 'success',
                summary: 'Success',
                detail: 'Franchise updated successfully',
                life: 3000
            });
        } else {
            ...
            toast.add({
                severity: 'success',
                summary: 'Success',
                detail: 'Franchise created successfully',
                life: 3000
            });
        }
        ...
        toast.add({
            severity: 'error',
            summary: 'Error',
            detail: error.response?.data?.error || 'Failed to save franchise',
            life: 3000
        });
    } finally {
        loading.value = false;
...
const deletePlaylist = async () => {
    try {
        ...
        toast.add({
            severity: 'error',
            summary: 'Error',
            detail: error.response?.data?.error || 'Failed to delete franchise',
            life: 3000
        });
    } finally {
        loading.value = false;
...
const openMediaDialog = async (playlist) => {
    ...
    if (playlist.has_children) {
        toast.add({
            ...
            detail: 'Media can only be added to franchises without children. This franchise contains child playlists.',
            life: 5000
        });
        return;
    }
...
const addMediaToPlaylist = async () => {
    ...
    try {
        ...
        toast.add({
            severity: 'success',
            summary: 'Success',
            detail: `Added ${selectedMediaToAdd.value.length} media to franchise`,
            life: 3000
        });
...
const removeMediaFromPlaylist = async () => {
    ...
    try {
        ...
        toast.add({
            severity: 'success',
            summary: 'Success',
            detail: `Removed ${selectedMediaToRemove.value.length} media from franchise`,
            life: 3000
        });
...
<template>
    <div class="card">
        ...
        <div class="flex justify-between items-center mb-6">
            <h5 class="mb-0">Franchises Management</h5>
            <Button label="Add Franchise" icon="pi pi-plus" @click="openCreateDialog" />
        </div>
            ...
            <div v-for="playlist in playlists" :key="playlist.id" class="col-span-12 sm:col-span-6 md:col-span-4 xl:col-span-3">
                <div class="card mb-0 p-4">
                    <div class="relative mb-4">
                        ...
                        <Tag value="Franchise" severity="info" class="absolute top-2 right-2" />
                    </div>
                    <div class="mb-3">
                    ...
                    <div class="flex gap-2">
                        ...
                        <Button v-if="playlist.has_children" icon="pi pi-folder-open" outlined rounded severity="success" size="small" @click="viewChildren(playlist)" title="View Children" />
                        <Button v-else icon="pi pi-video" outlined rounded severity="info" size="small" @click="openMediaDialog(playlist)" title="Manage Media" />
        ...
        <div v-if="!loading && playlists.length === 0" class="flex flex-col items-center justify-center py-12">
            ...
            <p class="text-xl text-muted-color mb-4">No franchises found</p>
            <Button label="Create Your First Franchise" icon="pi pi-plus" @click="openCreateDialog" />
        </div>
        ...
        <Paginator
            ...
            currentPageReportTemplate="Showing {first} to {last} of {totalRecords} franchises"
        ></Paginator>

        <!-- Create/Edit Dialog -->
        <Dialog v-model:visible="displayDialog" :header="isEditMode ? 'Edit Franchise' : 'Create Franchise'" :modal="true" :closable="true" class="p-fluid" style="width: 600px">
            <div class="flex flex-col gap-6 py-4">
                ...
                <div class="flex flex-col gap-2">
                    ...
                    <AutoComplete id="parent" v-model="formData.parent_id" :suggestions="filteredPlaylists" @complete="searchPlaylists" optionLabel="name" placeholder="Search parent franchise..." :loading="loadingPlaylists">
                    ...
                    <small class="text-muted-color">Leave empty to make this a root-level franchise</small>
                </div>
        ...
        <Dialog v-model:visible="displayDeleteDialog" header="Confirm Delete" :modal="true" :closable="true" style="width: 450px">
            <div class="flex items-center gap-4">
                ...
                <span v-if="selectedPlaylist">
                    Are you sure you want to delete franchise <b>{{ selectedPlaylist.name }}</b
                    >?
                </span>
            </div>
        ...
        <Dialog v-model:visible="displayMediaDialog" header="Manage Franchise Media" :modal="true" :closable="true" class="p-fluid" style="width: 900px; max-width: 95vw">
            ...
            <div class="grid grid-cols-2 gap-4">
                ...
                <div class="col-span-2 md:col-span-1">
                    <div class="flex justify-between items-center mb-3">
                        <h6 class="mb-0">Media in Franchise ({{ playlistMedia.length }})</h6>
                    ...
                    <p v-if="!loadingMedia && playlistMedia.length === 0" class="text-center text-muted-color py-4">No media in this franchise</p>
                </div>
            </div>
